...
def summarize(containers: list[ReceivingContainer]) -> tuple[Decimal, Decimal, Decimal]:
    total_volume = sum((item.volume_l for item in containers), Decimal("0"))
    absolute_alcohol = sum(
        (
            item.volume_l * item.corrected_abv / Decimal("100")
            for item in containers
        ),
        Decimal("0"),
    )
    average_abv = (
        round_abv(absolute_alcohol / total_volume * Decimal("100"))
        if total_volume > 0
        else Decimal("0")
    )
    return round_amount(total_volume), average_abv, round_amount(absolute_alcohol)


def container_line(item: ReceivingContainer) -> str:
    return (
        f"🫙 {item.container_id}. {format_decimal(item.volume_l)} л · "
        f"{format_decimal(item.observed_abv)}% · "
        f"{format_decimal(item.temperature_c)} °C -> "
        f"~{format_decimal(item.corrected_abv)}%"
    )


def containers_text(
    containers: list[ReceivingContainer],
    legacy: DrinkEvent | None = None,
) -> str:
    ...
    if not containers:
        lines.extend(
            [
                "",
                "Ёмкости пока не добавлены.",
                "Добавляйте их по мере сбора спирта-сырца.",
            ]
        )
...
async def write_summary(session: AsyncSession, process_id: int) -> None:
...
async def save_container(
    ...
    *,
    ...
    await session.flush()
    await write_summary(session, process_id)
...
async def get_owned_first_distillation(
    session: AsyncSession,
    *,
    process_id: int,
    telegram_id: int,
...
async def show_containers(
    ...
    async with session_factory() as session:
        process, containers = await get_owned_first_distillation(
            session,
            process_id=process_id,
            telegram_id=callback.from_user.id,
        )
        legacy = (
            await get_legacy_result(session, process_id)
    ...
    if stage_type_for_title(process.current_stage) != "first_distillation":
        await state.clear()
        await callback.answer("Ёмкости доступны на этапе первой перегонки.", show_alert=True)
        return
    ...
    await callback.message.edit_text(
        ...
        reply_markup=first_distillation_keyboard(
            process_id,
            [
                (item.container_id, f"🫙 {item.container_id}")
                for item in containers
            ],
        ),
    )
...
async def add_container_handler(
    callback: CallbackQuery,
    state: FSMContext,
    session_factory,
) -> None:
...
async def add_temperature_handler(
    message: Message,
    state: FSMContext,
    session_factory,
) -> None:
    ...
    await message.answer(
        ...
        reply_markup=first_distillation_container_keyboard(
            process_id,
...
async def container_handler(
    ...
    if process is None:
        ...
        return
    item = next(
        (item for item in containers if item.container_id == container_id),
        None,
    )
    if item is None:
        await show_containers(callback, state, session_factory, process_id)
...
async def edit_container_handler(
    callback: CallbackQuery,
    state: FSMContext,
    session_factory,
) -> None:
...
async def edit_container_value_handler(
    message: Message,
    state: FSMContext,
    session_factory,
) -> None:
    ...
    if (
        ...
        or field not in {"volume", "abv", "temperature"}
    ):
        await state.clear()
        return
    ...
    if (
        field == "temperature"
        and not MIN_TEMPERATURE_C <= value <= MAX_TEMPERATURE_C
    ):
    ...
    async with session_factory() as session:
        ...
        observed_abv = item.observed_abv
...
async def delete_container_handler(
    callback: CallbackQuery,
    state: FSMContext,
    session_factory,
) -> None:
    ...
    if process is None:
        ...
        await render_process_list(callback, session_factory)
        return
    if not any(item.container_id == container_id for item in containers):
        await show_containers(callback, state, session_factory, process_id)
        return
...
async def delete_container_confirm_handler(
    callback: CallbackQuery,
    state: FSMContext,
    session_factory,
) -> None:
...
async def process_calculators_handler(
    ...
    await state.clear()
    await callback.message.edit_text(
        calculator_menu_text(),
        reply_markup=first_distillation_calculators_keyboard(process_id),
    )


@router.callback_query(F.data == "calculators:first-distillation")
async def global_calculators_handler(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    await callback.answer()
    ...
    await callback.message.edit_text(
        calculator_menu_text(),
        reply_markup=first_distillation_calculators_keyboard(),
    )


@router.callback_query(
    F.data.regexp(r"^process:first-distillation-calc:\d+:(correction|absolute|blend)$")
)
async def process_calculator_start(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    parts = (callback.data or "").split(":")
...
@router.callback_query(
    F.data.regexp(r"^calculators:first-distillation:(correction|absolute|blend)$")
)
async def global_calculator_start(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    await start_calculator(
        callback,
        state,
        None,
        (callback.data or "").rsplit(":", 1)[-1],
    )
...
async def start_calculator(
    ...
    if kind == "correction":
        ...
        text = (
            ...
            "Введите каждую часть с новой строки в формате "
            "<code>объём крепость</code>.\n"
            "Например:\n"
            "<code>2,5 48\n2,5 32\n1,8 18</code>\n\n"
    ...
    await callback.message.edit_text(
        text,
        reply_markup=first_distillation_calculator_input_keyboard(process_id),
    )
...
def parse_blend(text: str) -> list[tuple[Decimal, Decimal]] | None:
    ...
    for line in (line.strip() for line in text.splitlines() if line.strip()):
        ...
        if len(tokens) != 2:
            return None
        volume = parse_positive_decimal(tokens[0])
        abv = parse_decimal(tokens[1].rstrip("%"))
        if (
            volume is None
            or abv is None
            or not Decimal("0") <= abv <= MAX_ABV
        ):
            return None
...
async def calculator_value_handler(
    message: Message,
    state: FSMContext,
) -> None:
    if message.text is None:
        return
    data = await state.get_data()
    kind = data.get("calc_kind")
    step = data.get("calc_step")
    process_id = data.get("calc_process_id")
    keyboard_process_id = process_id if isinstance(process_id, int) else None

    if kind == "blend":
        blend = parse_blend(message.text)
        if blend is None:
            await message.answer(
                "Введите минимум две строки: <code>объём крепость</code>."
            )
            return
        total_volume = sum((volume for volume, _ in blend), Decimal("0"))
        total_aa = sum(
            (
                volume * abv / Decimal("100")
                for volume, abv in blend
            ),
            Decimal("0"),
        )
    ...
    if kind == "correction":
        ...
        await message.answer(
            "🌡 <b>Поправка спиртометра</b>\n\n"
            f"Показание: {format_decimal(observed)}% при "
            f"{format_decimal(value)} °C\n"
    ...
    if kind == "absolute":
        ...
        await message.answer(
            "💧 <b>Абсолютный спирт</b>\n\n"
            f"Объём: {format_decimal(volume)} л · "
            f"Крепость: {format_decimal(value)}%\n"
