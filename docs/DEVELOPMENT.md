...
В проекте не используется Unicode em dash (`U+2014`). Это правило распространяется на пользовательские сообщения, экраны, кнопки, подсказки, тексты калькуляторов и исходный код бота.

Для тире и текстовых разделителей используется обычный символ `-`.
